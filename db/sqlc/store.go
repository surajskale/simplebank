...
type Store struct {
	*Queries         // all individul functions provided by queries are supported by struct, this is composition
...
func (store *Store) execTx(ctx context.Context, fn func(*Queries) error) error { // here execTx starts with smallcase that's why it's not exported another package can not call it, it is only for internal use
	...
	if err != nil {
		if rbErr := tx.Rollback(); rbErr != nil {
			return fmt.Errorf("tx error: %v, rb error: %v", err, rbErr) // if there is error while rolling back return both errors
		}
		return err // no error while rollback
	}
...
var txKey = struct{}{} // declaring txkey as empty struct to 	use as name
...
func addMoney(
	...
	if err != nil {
		return // same as return account1, account2, amount
	}
